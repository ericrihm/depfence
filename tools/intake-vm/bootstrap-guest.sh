...
GVISOR_SIGNING_FINGERPRINT="6F1DF85E3A71C24918E727D56FC6D554E32BD943"
...
gvisor_key="$(mktemp)"
trap 'rm -f "$gvisor_key"' EXIT HUP INT TERM
curl --fail --location --proto '=https' --tlsv1.2 \
    --output "$gvisor_key" https://gvisor.dev/archive.key
actual_gvisor_fingerprint="$(gpg --show-keys --with-colons "$gvisor_key" \
    | awk -F: '$1 == "fpr" {print $10; exit}')"
test "$actual_gvisor_fingerprint" = "$GVISOR_SIGNING_FINGERPRINT" || {
    printf '%s\n' 'gVisor signing-key fingerprint mismatch' >&2
    exit 65
}
sudo gpg --dearmor --yes -o /usr/share/keyrings/gvisor-archive-keyring.gpg "$gvisor_key"
rm -f "$gvisor_key"
