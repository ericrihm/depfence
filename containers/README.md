...
- `render` permits only rendered PDF/DOCX/local-HTML comparison and includes LibreOffice,
...
Worker images are published independently of PyPI or a GitHub release, only by
manual dispatch from reviewed `main`, as immutable `sha-<commit>` GHCR digests. The workflow
...
After publishing the reviewed commit's workers, copy an archive of that exact
commit into the VM and run `tools/intake-vm/readiness.sh` there with the signed
intake/static/render image digests. It verifies signatures and attestations,
runs `intake doctor`, executes the proxy and
