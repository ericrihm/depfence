...
def _scan_docx(path: Path, data: bytes) -> list[Finding]:
    ...
    with archive:
        ...
        document = _bounded_zip_member(archive, document_info, MAX_XML_BYTES)
        root = _parse_bounded_xml(document)
        ...
        for run in root.iter():
            if run.tag.rsplit("}", 1)[-1] != "r":
                continue
            text_value = "".join(
                node.text or "" for node in run.iter() if node.tag.rsplit("}", 1)[-1] == "t"
            )
            font = None
            for node in run.iter():
                if node.tag.rsplit("}", 1)[-1] == "rFonts":
                    font = next((str(value) for key, value in node.attrib.items() if key.rsplit("}", 1)[-1] in {"ascii", "hAnsi"}), None)
                    break
            if font:
...
def _scan_pdf(path: Path, data: bytes) -> list[Finding]:
    invisible = len(_PDF_INVISIBLE_TEXT.findall(data))
    images = data.count(b"/Subtype /Image") + data.count(b"/Subtype/Image")
    text_ops = len(_PDF_TEXT_SHOW.findall(data))
    if invisible == 0 or images == 0 or text_ops == 0:
        return []
    return [_finding(
        path,
        "DF-PDF-001",
        "PDF contains an invisible text layer over imagery",
        "The PDF combines image content with invisible selectable text. This is common for legitimate OCR and is not proof of deception without rendered-versus-extracted comparison.",
        Severity.MEDIUM,
        0.62,
...
def scan_artifact_bytes(path: Path, data: bytes) -> tuple[list[Finding], list[str]]:
    ...
    if suffix in FONT_SUFFIXES or media_type.startswith("font/"):
        ...
        errors = [] if summary.complete else [
            f"{summary.flavor} requires sandboxed deep analysis"
        ]
        return [], errors
...
def _sandbox_findings(document: object, artifact_name: str) -> list[Finding]:
    ...
    for raw in document["findings"]:
        ...
        if rule_id in seen_rules:
            raise ScanIncompleteError("sandbox returned a duplicate finding")
        ...
        if (
            ...
            or set(metrics) != required_metric_keys
